...
class Authenticator {
  ...
  authenticate () {
    return async function (req, res, next) {
      ...
      if (req.authentication && this.options.validate) req.authentication = await this.options.validate(req.authentication, req.body)
      ...
      res.send = async function () {
        ...
        if (req.authenticated && this.options.reissue) {
          let authentication = await this.options.reissue(req.authentication)
